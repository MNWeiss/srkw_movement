...
transformed parameters {
  ...
  for(m in 1:M){
    ...
    log_omega[m, 1, d] = bernoulli_lpmf(S[d, m] | mu[1]);
    log_omega[m, 2, d] = bernoulli_lpmf(S[d, m] | mu[2]);
    log_omega[m, 3, d] = bernoulli_lpmf(S[d, m] | mu[3]);
    }
  }
}
...
model {
  ...
  pd ~ normal(0, 10); // Uniform priors seem to make it struggle to initialise. 
  parrive ~  normal(0, 10); 
  pleave ~ normal(0, 10); 
  ...
  for(m in 1:M){
  target += hmm_marginal(log_omega[m, ,], Gamma, rho[m,]);
  }
}
...
generated quantities {
  ...
  int y_sim[M, D];
  for(m in 1:M){
      hidden_probs[m,,] = hmm_hidden_state_prob(log_omega[m,,], Gamma, rho[m,]);
      y_sim[m,] = hmm_latent_rng(log_omega[m,,], Gamma, rho[m,]);
  }
